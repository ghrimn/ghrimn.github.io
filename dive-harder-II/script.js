...
const selectedStratagemsState = stratagems.reduce((acc, stratagem) => {
...
function createStratagemIcon(stratagem) {
    ...
    stratagemIcon.classList.add('stratagem-btn', 'selected');
...
function createEquipmentIcon(item, type) {
    ...
    equipmentIcon.classList.add('equipment-btn', 'selected');
...
types.forEach(type => {
    ...
    const items = equipment.flatMap(equipmentItem =>
        equipmentItem.items.filter(item => item.type === type)
    );
...
function getRandomEquipment(type, currentEquipment) {
    let itemsOfType = equipmentItems.filter(item => item.type === type && !selectedEquipmentState[`${type}:${item.name}`]);

    // Filter out the current equipment for this type
    if (currentEquipment) {
        itemsOfType = itemsOfType.filter(item => item.name !== currentEquipment.name);
    }
...
function getRandomStratagems(stratagems, count, index) {
    ...
    if (currentStratagems.length > 0) {
        console.log(currentStratagems.filter((_, i) => i !== index))
        currentStratagems.filter((_, i) => i !== index).forEach(stratagem => {
...
function singleRandomStratagem(index) {
    ...
    console.log(availableStratagems)
...
document.getElementById("difflvlResult").addEventListener("click", () => { diffgen(); });
